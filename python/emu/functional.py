...
def cached(cache, scoped=True, typed=False, key=None):
    ...
    that the function wrapped should take only hashable arguments.

    :param cache: Any :class:`dict`-like object suitable for
    ...
    :type cache: Mapping
    :param scoped: Whether the function itself is part of the
    ...
    :type scoped: bool
    :param typed: Whether to factor argument types into the cache
        check. Default ``False``, setting to ``True`` causes the
        cache keys for ``3`` and ``3.0`` to be considered unequal.
    :type typed: bool
    :return: A function which caches the results of the wrapped
        function.
    :rtype: callable
...
def rpartial(fun, *args2, **kwargs2):
    '''new function with reverse partial application
...
def boost_fn(fn):
    ...
    with another callable.

    A short alias is available named `bf`.

    :param fn: a callable
    :type fn: callable
    :return: the provided callable wrapped with enhanced operators
    :rtype: callable
