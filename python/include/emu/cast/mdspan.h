...
namespace emu::cast
{
    ...
    struct python_adaptor<Mdspan> : std::true_type
    {

        // using cpp_type = emu::detail::mdspan<ElementType, Location, Extents, LayoutPolicy, AccessorPolicy>;
        using cpp_type = Mdspan;

        using element_type         = typename cpp_type::element_type;
        using location             = location_or_default_type<cpp_type>;
        using extents_type         = typename cpp_type::extents_type;
        using layout_policy_type   = typename cpp_type::layout_policy_type;
        using accessor_policy_type = typename cpp_type::accessor_policy_type;

        using bia = buffer_info_adaptor<element_type, location>;
        ...
        emu::optional<cpp_type> convert(py::handle handle, bool can_convert) {

            // Try cast handle into buffer.
            return bia_.get_buffer_info(handle, can_convert).and_then([](auto buffer_info) -> emu::optional<cpp_type> {
                ...
                return layout_adaptor<element_type, extents_type, layout_policy_type>::mapping(buffer_info).and_then([&] (auto&& extents) {
                    if constexpr (cpts::has_location<cpp_type>) {
                        return emu::optional<cpp_type>(in_place,
...
namespace pybind11::detail
{
    ...
    {
        // using adaptor  = emu::cast::python_adaptor<emu::detail::mdspan<ElementType, Location, Extents, LayoutPolicy, AccessorPolicy>>;
        using adaptor  = emu::cast::python_adaptor<Mdspan>;
