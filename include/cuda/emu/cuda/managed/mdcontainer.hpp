...
namespace emu::cuda::managed
{
    ...
    struct mdcontainer : emu::detail::basic_mdcontainer<
        T, Extents, LayoutPolicy, AccessorPolicy, emu::cuda::managed_location_policy,
        mdcontainer<T, Extents, LayoutPolicy, AccessorPolicy>
    >
    {
        using base = emu::detail::basic_mdcontainer< T, Extents, LayoutPolicy, AccessorPolicy, emu::cuda::managed_location_policy, mdcontainer >;
    ...
    make_mdcontainer(const device_t& device, OtherIndexTypes... exts){
        ...
        size_t size = (1 * ... * exts);
        auto u_ptr = emu::cuda::managed::make_unique<T[]>(device, size);
