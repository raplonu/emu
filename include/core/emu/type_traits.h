...
namespace detail
{
    ...
    using ForwardConst = std::conditional_t<std::is_const_v<std::remove_reference_t<T>>, std::add_const_t<std::remove_reference_t<U>>, U>;

    template<typename T>
    using ValueType      = typename std::remove_reference_t<T>::value_type;
    template<typename T>
    using KeyType        = typename std::remove_reference_t<T>::key_type;
    template<typename T>
    using MappedType     = typename std::remove_reference_t<T>::mapped_type;
    template<typename T>
    using DifferenceType = typename std::remove_reference_t<T>::difference_type;
    template<typename T>
    using Category       = typename std::remove_reference_t<T>::iterator_category;

    template<typename It>
    using IteratorValue      = ValueType<std::iterator_traits<std::remove_reference_t<It>>>;
    template<typename It>
    using IteratorDifference = DifferenceType<std::iterator_traits<std::remove_reference_t<It>>>;
    template<typename It>
    using IteratorCategory   = Category<std::iterator_traits<std::remove_reference_t<It>>>;
