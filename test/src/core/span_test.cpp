...
TEST(span_test, from_std_array_constructor)
{
...

    {
        ...
        { return {1, 2, 3, 4}; };
        auto take_a_span = [](emu::span<int> s)
        { static_cast<void>(s); };
...

    {
        auto get_an_array = []() -> std::array<int, 4>
        { return {1, 2, 3, 4}; };
        auto take_a_span = [](emu::span<const int> s)
        { static_cast<void>(s); };
        // try to take a temporary std::array
        take_a_span(get_an_array());
    }
...
TEST(span_test, from_container_constructor)
{
...

    {
         auto get_temp_string = []() -> std::string
         { return {}; };
         auto use_span = [](emu::span<const char> s)
         { static_cast<void>(s); };
         use_span(get_temp_string());
    }
    ...
    {
         auto get_temp_string = []() -> const std::string
         { return {}; };
         auto use_span = [](emu::span<const char> s)
         { static_cast<void>(s); };
         use_span(get_temp_string());
...
TEST(span_test, copy_move_and_assignment)
{
    ...
    auto get_temp_span = [&]() -> emu::span<int>
    { return {&arr[1], 2}; };
    auto use_span = [&](emu::span<const int> s)
    {
        EXPECT_TRUE(s.size() == 2);
...
TEST(span_test, as_bytes)
{
    ...
    int a[] = {1, 2, 3, 4};
    {
         ...
         const emu::span<const byte> bs = emu::as_bytes(s);
...
TEST(span_test, as_writable_bytes)
{
...

    {
         ...
         EXPECT_TRUE(s.size() == 4);
         emu::span<const byte> bs = emu::as_writable_bytes(s);
    ...
    {
         emu::span<int> s;
         const auto bs = emu::as_writable_bytes(s);
         EXPECT_TRUE(bs.size() == s.size());
         EXPECT_TRUE(bs.size() == 0);
         EXPECT_TRUE(bs.size_bytes() == 0);
         EXPECT_TRUE(static_cast<void *>(bs.data()) == static_cast<void *>(s.data()));
