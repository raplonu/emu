...
TEST(span_test, from_nullptr_size_constructor)
{
    ...
    {
        emu::span<int> s{nullptr, narrow_cast<emu::span<int>::size_type>(0)};
        EXPECT_TRUE(s.size() == 0);
        EXPECT_TRUE(s.data() == nullptr);

        emu::span<int> cs{nullptr, narrow_cast<emu::span<int>::size_type>(0)};
        EXPECT_TRUE(cs.size() == 0);
        EXPECT_TRUE(cs.data() == nullptr);
    }
    {
        auto workaround_macro = []() {
            const emu::span<int, 1> s{nullptr, narrow_cast<emu::span<int>::size_type>(0)};
        };
        EXPECT_DEATH(workaround_macro(), deathstring);
    }
    {
        auto workaround_macro = []() { const emu::span<int> s{nullptr, 1}; };
        EXPECT_DEATH(workaround_macro(), deathstring);

        auto const_workaround_macro = []() { const emu::span<const int> s{nullptr, 1}; };
        EXPECT_DEATH(const_workaround_macro(), deathstring);
    }
    {
        auto workaround_macro = []() { const emu::span<int, 0> s{nullptr, 1}; };
        EXPECT_DEATH(workaround_macro(), deathstring);

        auto const_workaround_macro = []() { const emu::span<const int, 0> s{nullptr, 1}; };
        EXPECT_DEATH(const_workaround_macro(), deathstring);
    }
    {
        emu::span<int*> s{nullptr, narrow_cast<emu::span<int>::size_type>(0)};
        ...
        emu::span<const int*> cs{nullptr, narrow_cast<emu::span<int>::size_type>(0)};
        EXPECT_TRUE(cs.size() == 0);
        EXPECT_TRUE(cs.data() == nullptr);
    }
}
...
TEST(span_test, from_pointer_length_constructor)
{
...

    {
        ...
        {
            {
                emu::span<int> s = {&arr[0], narrow_cast<std::size_t>(i)};
            ...
            {
                emu::span<int> s = {&arr[i], 4 - narrow_cast<std::size_t>(i)};
    ...
    {
        int* p = nullptr;
        emu::span<int> s{p, narrow_cast<emu::span<int>::size_type>(0)};
        EXPECT_TRUE(s.size() == 0);
        EXPECT_TRUE(s.data() == nullptr);
    }
    ...
    {
        int* p = nullptr;
        auto workaround_macro = [=]() { const emu::span<int> s{p, 2}; };
        EXPECT_DEATH(workaround_macro(), deathstring);
    }
}
...
TEST(span_test, from_pointer_pointer_construction)
{
    ...
    {
        emu::span<int> s{&arr[0], &arr[0]};
        EXPECT_TRUE(s.size() == 0);
        EXPECT_TRUE(s.data() == &arr[0]);
    }

    {
        emu::span<int, 0> s{&arr[0], &arr[0]};
        EXPECT_TRUE(s.size() == 0);
        EXPECT_TRUE(s.data() == &arr[0]);
    }
    ...
    //{
    //    auto workaround_macro = [&]() { emu::span<int> s{&arr[1], &arr[0]}; };
    //    EXPECT_DEATH(workaround_macro(), deathstring);
    //}
    ...
    //{
    //    int* p = nullptr;
    //    auto workaround_macro = [&]() { emu::span<int> s{&arr[0], p}; };
    //    EXPECT_DEATH(workaround_macro(), deathstring);
    //}
...
TEST(span_test, from_array_constructor)
 {
     ...
     {
         const emu::span<int> s{arr};
         EXPECT_TRUE(s.size() == 5);
         EXPECT_TRUE(s.data() == &arr[0]);
     }

     {
         const emu::span<int, 5> s{arr};
         EXPECT_TRUE(s.size() == 5);
         EXPECT_TRUE(s.data() == &arr[0]);
     }
     ...
     {
         emu::span<int, 6> s{arr};
     }

     {
         emu::span<int, 0> s{arr};
         EXPECT_TRUE(s.size() == 0);
         EXPECT_TRUE(s.data() == &arr[0]);
     }

     {
         emu::span<int> s{arr2d};
     ...
     {
         emu::span<int, 0> s{arr2d};
         EXPECT_TRUE(s.size() == 0);
         EXPECT_TRUE(s.data() == &arr2d[0][0]);
     }

     {
         emu::span<int, 6> s{arr2d};
     }
 #endif
     {
         const emu::span<int[3]> s{std::addressof(arr2d[0]), 1};
     ...
     {
         emu::span<int, 0> s{arr3d};
         EXPECT_TRUE(s.size() == 0);
         EXPECT_TRUE(s.data() == &arr3d[0][0][0]);
     }

     {
         emu::span<int, 11> s{arr3d};
     }

     {
         emu::span<int, 12> s{arr3d};
         EXPECT_TRUE(s.size() == 12);
         EXPECT_TRUE(s.data() == &arr3d[0][0][0]);
         EXPECT_TRUE(s[0] == 1);
         EXPECT_TRUE(s[5] == 6);
 ...
 #endif
     {
         const emu::span<int[3][2]> s{std::addressof(arr3d[0]), 1};
         EXPECT_TRUE(s.size() ==  1);
     }
     ...
     {
         const emu::span<AddressOverloaded, 5> s{ao_arr};
         EXPECT_TRUE(s.size() == 5);
 ...
 TEST(span_test, from_dynamic_array_constructor)
 {
     ...
     {
         emu::span<double> s(&arr[0][0][0], 10);
 ...
 TEST(span_test, from_std_array_constructor)
 {
...

     {
         ...
         emu::span<const int> cs{arr};
         EXPECT_TRUE(cs.size() == arr.size());
         EXPECT_TRUE(cs.data() == arr.data());
     }

     {
         emu::span<int, 4> s{arr};
         EXPECT_TRUE(s.size() == arr.size());
         EXPECT_TRUE(s.data() == arr.data());

         emu::span<const int, 4> cs{arr};
         EXPECT_TRUE(cs.size() == arr.size());
         EXPECT_TRUE(cs.data() == arr.data());
     }
     ...
     {
         emu::span<AddressOverloaded, 4> fs{ao_arr};
 ...
 #ifdef CONFIRM_COMPILATION_ERRORS
     {
         emu::span<int, 2> s{arr};
         EXPECT_TRUE(s.size() == 2);
         EXPECT_TRUE(s.data() == arr.data());

         emu::span<const int, 2> cs{arr};
...

     {
         ...
         emu::span<const int, 0> cs{arr};
         EXPECT_TRUE(cs.size() == 0);
         EXPECT_TRUE(cs.data() == arr.data());
     }

     {
         emu::span<int, 5> s{arr};
     }

     {
         auto get_an_array = []() -> std::array<int, 4> { return {1, 2, 3, 4}; };
         auto take_a_span = [](emu::span<int> s) { static_cast<void>(s); };
     ...
     {
         auto get_an_array = []() -> std::array<int, 4> { return {1, 2, 3, 4}; };
         auto take_a_span = [](emu::span<const int> s) { static_cast<void>(s); };
         // try to take a temporary std::array
         take_a_span(get_an_array());
     }
 }
 ...
 TEST(span_test, from_const_std_array_constructor)
 {
     ...
     {
         emu::span<const int> s{arr};
         EXPECT_TRUE(s.size() == arr.size());
         EXPECT_TRUE(s.data() == arr.data());
     }

     {
         emu::span<const int, 4> s{arr};
         EXPECT_TRUE(s.size() == arr.size());
         EXPECT_TRUE(s.data() == arr.data());
     }
     ...
     {
         emu::span<const AddressOverloaded, 4> s{ao_arr};
 ...
 #ifdef CONFIRM_COMPILATION_ERRORS
     {
         emu::span<const int, 2> s{arr};
         EXPECT_TRUE(s.size() == 2);
         EXPECT_TRUE(s.data() == arr.data());
     }

     {
         emu::span<const int, 0> s{arr};
         EXPECT_TRUE(s.size() == 0);
         EXPECT_TRUE(s.data() == arr.data());
     }

     {
         emu::span<const int, 5> s{arr};
     }
 #endif
 ...
 TEST(span_test, from_container_constructor)
 {
     ...
     {
 #ifdef CONFIRM_COMPILATION_ERRORS
         emu::span<char> s{cstr};
 #endif
         emu::span<const char> cs{cstr};
...

     {
         ...
         auto use_span = [](emu::span<int> s) { static_cast<void>(s); };
     ...
     {
         auto get_temp_vector = []() -> std::vector<int> { return {}; };
         auto use_span = [](emu::span<const int> s) { static_cast<void>(s); };
         use_span(get_temp_vector());
     }

     {
         ...
         auto use_span = [](emu::span<char> s) { static_cast<void>(s); };
     ...
     {
         auto get_temp_string = []() -> std::string { return {}; };
         auto use_span = [](emu::span<const char> s) { static_cast<void>(s); };
         use_span(get_temp_string());
     }
 ...
 TEST(span_test, from_convertible_span_constructor){{emu::span<DerivedClass> avd;
 emu::span<const DerivedClass> avcd = avd;
 static_cast<void>(avcd);
 }

 {
 #ifdef CONFIRM_COMPILATION_ERRORS
     emu::span<DerivedClass> avd;
     emu::span<BaseClass> avb = avd;
 ...
 {
     emu::span<int> s;
     emu::span<unsigned int> s2 = s;
     static_cast<void>(s2);
 }

 {
     emu::span<int> s;
     emu::span<const unsigned int> s2 = s;
     static_cast<void>(s2);
 }
 ...
 TEST(span_test, copy_move_and_assignment)
 {
     ...
     auto get_temp_span = [&]() -> emu::span<int> { return {&arr[1], 2}; };
     auto use_span = [&](emu::span<const int> s) {
         EXPECT_TRUE(s.size() ==  2);
 ...
 {
     emu::span<int>::iterator it1;
     emu::span<int>::iterator it2;
     EXPECT_TRUE(it1 == it2);
 }
 ...
 TEST(span_test, iterator_comparisons)
 {
     ...
     {
         emu::span<int> s = a;
         emu::span<int>::iterator it = s.begin();
         auto it2 = it + 1;
 ...
 TEST(span_test, begin_end)
 {
    std::set_terminate([] {
    ...
    });
     {
         ...
         emu::span<int>::iterator it = s.begin();
         emu::span<int>::iterator it2 = std::begin(s);
         EXPECT_TRUE(it == it2);
 ...
 TEST(span_test, as_bytes)
 {
     ...
     int a[] = {1, 2, 3, 4};
     {
         const emu::span<const int> s = a;
         EXPECT_TRUE(s.size() == 4);
         const emu::span<const byte> bs = as_bytes(s);
     ...
     int b[5] = {1, 2, 3, 4, 5};
     {
         emu::span<int> sp(begin(b), static_cast<size_t>(-2));
 ...
 TEST(span_test, as_writable_bytes)
 {
...

     {
         ...
         emu::span<const int> s = a;
         EXPECT_TRUE(s.size() == 4);
         emu::span<const byte> bs = as_writable_bytes(s);
 ...
 // initialization or assignment to static span that REDUCES size is NOT ok
     ...
     {
         const emu::span<int, 2> s2{&arr[0], 2};
         static_cast<void>(s2);
     }
     {
         const emu::span<int, 1> s1 = s4.first<1>();
         static_cast<void>(s1);
     }

    /*
     // this is not a legal operation in std::span, so we are no longer supporting it
     // conversion from emu::span<int, 4> to emu::span<int, dynamic_extent> via call to `first`
     // then convert from emu::span<int, dynamic_extent> to emu::span<int, 1>
     // The dynamic to fixed extents are not supported in the standard
     // to make this work, emu::span<int, 1> would need to be emu::span<int>.
     {

         // NB: implicit conversion to emu::span<int,1> from emu::span<int>
         emu::span<int, 1> s1 = s4.first(1);
         static_cast<void>(s1);
     }
     */
 ...
 #ifdef CONFIRM_COMPILATION_ERRORS
     {
         emu::span<int, 4> s3 = arr2;
     }
     {
         emu::span<int, 2> s2 = arr2;
         emu::span<int, 4> s4a = s2;
     }
 #endif
     {
         auto f = [&]() {
             const emu::span<int, 4> _s4{arr2, 2};
    ...
    /*
     ...
     emu::span<int> av = arr2; auto f = [&]() {
         const emu::span<int, 4> _s4 = av;
         static_cast<void>(_s4);
     };
     EXPECT_DEATH(f(), deathstring);
     */
 ...
 TEST(span_test, interop_with_std_regex)
 {
     ...
     std::match_results<emu::span<char>::iterator> match;
 ...
 {
     EXPECT_TRUE((std::is_default_constructible<emu::span<int>>::value));
     EXPECT_TRUE((std::is_default_constructible<emu::span<int, 0>>::value));
     EXPECT_FALSE((std::is_default_constructible<emu::span<int, 42>>::value));
 }
