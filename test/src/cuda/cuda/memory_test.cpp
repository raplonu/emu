...
{
    auto device = emu::cuda::current_device();
...
{
    auto device = emu::cuda::current_device();
    auto stream = emu::cuda::stream(device);
    ...
    emu::cuda::copy_bytes_n(host_src.get(), n, device_mem.get(), stream);

    auto host_dst = emu::cuda::host::make_unique<int[]>(n);
    emu::cuda::copy_bytes_n(device_mem.get(), n, host_dst.get(), stream);
...
TEST(CudaMemoryTest, SyncCopy)
{
    ...
    emu::cuda::copy_bytes_n(host_src.get(), n, device_mem.get());

    auto host_dst = emu::cuda::host::make_unique<int[]>(device, n);
    emu::cuda::copy_bytes_n(device_mem.get(), n, host_dst.get());
